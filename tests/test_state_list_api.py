...
def test_list_authenticated(state_factory, user_factory, api_client):
    """test the get request to view the list of states of a country identified by the country code in the url when authenticated"""
    ...
    url=endpoint + states[0].country.code+"/"                           # Get the url by adding country code of the first country instance to endpoint url
...
@pytest.mark.django_db
def test_list_not_authenticated(state_factory, api_client):
    """test the get request to view the list of states of a country identified by the country code in the url when not authenticated"""
...
@pytest.mark.django_db
def test_search(state_factory, user_factory, api_client):
    ...
    for state_instance in states:                                       # Determine the expected length for the specific country and name or check how many instances have the same country code & name as the first instance
        if state_instance.country.code == states[0].country.code:
